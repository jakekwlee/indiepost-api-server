...
@NamedNativeQueries(
        ...
        NamedNativeQuery(
                ...
                query = """select p.id, p.title, p.publishedAt, p.displayName author, c.name category, count(*) as pageviews, count(distinct s.visitorId) as uniquePageviews from Posts p left join Stats s on s.postId = p.id inner join Categories c on p.categoryId = c.id and p.status = 'PUBLISH' group by p.id order by p.publishedAt desc"""),
        NamedNativeQuery(
                name = "@GET_ALL_POST_STATS_FROM_CACHE",
                query = """select p.id, p.title, p.publishedAt, p.displayName author, c.name category, s.pageviews, s.uniquePageviews, s.legacyPageviews, s.legacyUniquePageviews  from CachedPostStats s inner join Posts p on s.postId = p.id inner join Categories c on p.categoryId = c.id order by p.publishedAt desc"""),
        NamedNativeQuery(
